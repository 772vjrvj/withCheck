...
<body>
	...
	<form action="insertCheck.do" method="post">
		...
		<table id="calendartitle" border="1" style="border: 3px solid #000000;">
			...
			<tr>
				<td colspan="3" style="color:#245682; text-align: center; font-size: 17px;">${dto.content}</td>            
			</tr>
		</table>
				...
				<table id="calendar" border="1" style="text-align: center;">
					...
					<c:forEach var = "n" begin = "1" end = "${Cal.lastDay(i,j,1)}">
						...
						<c:choose>
							...
							<c:otherwise>
				     			<p class="countview" style="color:${Util.fontColor(Cal.dayOfWeek(i,j,1), n)};">${n}</p>
		                		<input  class="option-input checkbox" type="checkbox" name="chk" value='${i+""}${Util.isTwo(j+"")}${Util.isTwo(n+"")}'
		                		${Util.checked(chkList, (i+""),Util.isTwo(j+""),Util.isTwo(n+""))} 
		                		${Util.today((i+""),Util.isTwo(j+""),Util.isTwo(n+""))}
		                		/>
							</c:otherwise>
						</c:choose>
	            		<c:choose>
